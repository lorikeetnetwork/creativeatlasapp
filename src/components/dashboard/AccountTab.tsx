...
import { CheckCircle, Clock, User, CreditCard, Building2, ArrowRight, Loader2, Settings, Calendar, Compass } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex-1 space-y-4 w-full">
              <div>
                <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-2">
                  <h3 className="text-xl md:text-2xl font-semibold text-foreground">{profile.full_name || 'User'}</h3>
              ...
              {profile.account_type === 'free' && (
                <div className="bg-muted/50 p-3 md:p-4 rounded-lg border border-border">
                  <p className="text-xs md:text-sm mb-3 text-muted-foreground">
                    <strong className="text-foreground">List your creative business</strong> on the map to reach more customers.
                  </p>
              ...
              {profile.account_type === 'creative_entity' && (
                <div className="space-y-2">
                  <p className="text-xs md:text-sm font-medium text-foreground">Your benefits:</p>
      ...
      <BentoContentCard
        ...
        headerActions={<CreditCard className="w-5 h-5 md:w-6 md:h-6 text-muted-foreground" />}
      >
        {payments.length === 0 ? (
          <div className="text-center py-8 md:py-12">
            <Clock className="w-10 h-10 md:w-12 md:h-12 text-muted-foreground mx-auto mb-4" />
            <p className="text-sm md:text-base text-muted-foreground mb-4">No payment history yet</p>
            {profile.account_type === 'free' && (
            <Button size="sm" onClick={() => navigate('/pricing')}>
              View Pricing Plans
            </Button>
          ...
          <div className="space-y-3 md:space-y-4">
            {payments.map((payment) => (
              <div key={payment.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 md:p-4 border border-border rounded-lg gap-3 bg-muted/50">
                ...
                <div className="text-left sm:text-right">
                  <p className="font-semibold text-sm md:text-base text-foreground">{formatAmount(payment.amount, payment.currency)}</p>
